...
import { Search, RefreshCw, DollarSign, Calendar, Package, TrendingDown, Percent, CheckCircle, XCircle } from "lucide-react"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { apiUrl } from "@/lib/env"

export default function DiscountLogPage() {
  ...
  // ใช้ Next.js API route ที่จะ proxy ไปยัง backend
  const discountLogEndpoint = "http://localhost:4000/api/discount-log"

  const fetchDiscountLogs = async (page: number = 1, limit: number = 50) => {
    try {
      setLoading(true)
      setError("")

      const url = `${discountLogEndpoint}?page=${page}&limit=${limit}`
      ...
      setDiscountLogs(dataArray)
      setPagination(paginationData)
      setLastRefresh(new Date())
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Unknown error"
      setError(`Network Error: ${errorMessage}`)
      console.error("❌ Error fetching discount logs:", err)
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    fetchDiscountLogs(currentPage, pageLimit)
  }, [currentPage, pageLimit])

  // Filter (client-side filtering after data is loaded)
  const filteredLogs = discountLogs.filter(
    (log) => {
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
            ประวัติการปรับลดราคา (Discount Log)
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            ข้อมูลราคาที่มีการปรับลดลงมาต่ำกว่าคู่แข่งของ JP จาก {discountLogEndpoint}
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="ค้นหาประวัติการปรับลดราคา..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-10"
                />
              </div>
              <Button onClick={() => fetchDiscountLogs(currentPage, pageLimit)} disabled={loading} variant="outline">
          ...
          <Card className="mb-6 border-red-200 bg-red-50">
            <CardContent className="p-6">
              <div className="text-red-600 text-center">
                ...
                <p>{error}</p>
                <Button onClick={() => fetchDiscountLogs(currentPage, pageLimit)} className="mt-4" variant="outline">
                  ลองใหม่
                </Button>
              </div>
            </CardContent>
          ...
          <>
            <div className="mb-4">
              <p className="text-sm text-gray-600">
                พบ{" "}
                <span className="font-semibold text-green-600">{filteredLogs.length}</span> รายการ
                {searchTerm && (
                  <span>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="w-[150px]">แพ็กเกจ</TableHead>
                          <TableHead className="w-[120px] text-right">ราคา JP</TableHead>
                          <TableHead className="w-[120px] text-right">ราคาเดิม</TableHead>
                          <TableHead className="w-[120px] text-right">ราคาใหม่</TableHead>
                          <TableHead className="w-[120px] text-right">ราคาคู่แข่ง</TableHead>
                          <TableHead className="w-[100px] text-right">ส่วนลด %</TableHead>
                          <TableHead className="w-[110px] text-right">จำนวนส่วนลด</TableHead>
                          <TableHead className="w-[100px]">สถานะ</TableHead>
                          <TableHead className="w-[120px]">แหล่งที่มา</TableHead>
                            ...
                            <TableRow key={`log-${log._id || index}`}>
                              ...
                              <TableCell className="text-right">
                                <span className="text-sm text-orange-600">
                                  ฿{formatPrice(log.metadata?.competitor_price || "N/A")}
                                </span>
                              </TableCell>
                              <TableCell className="text-right">
                                <div className="flex flex-col items-end">
                                  <span className={`text-sm font-semibold ${
                                    log.discount_percent > 0 ? "text-green-600" : "text-gray-600"
                                  }`}>
                                    {log.discount_percent ? `${log.discount_percent.toFixed(2)}%` : "0%"}
                                  </span>
                                  {log.discount_limit_percent && (
                                    <span className="text-xs text-gray-500">
                                      จำกัด: {log.discount_limit_percent}%
                                    </span>
                                  )}
                                </div>
                              </TableCell>
                              <TableCell className="text-right">
                                <span className={`text-sm font-semibold ${
                                  log.discount_amount > 0 ? "text-green-600" : "text-gray-600"
                                }`}>
                              ...
                              <TableCell>
                                <div className="flex flex-col gap-1">
                                  ...
                                  {log.reason && (
                                    <div className="text-xs text-gray-500 mt-1 max-w-[150px] truncate" title={log.reason}>
                                      {log.reason}
                                    </div>
                                  )}
                                </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-blue-50 rounded-lg p-3">
                            <div className="text-xs text-blue-600 mb-1">ราคา JP</div>
                            <div className="text-lg font-semibold text-blue-700">
                              ฿{formatPrice(log.jp_price)}
                            </div>
                          </div>
                          <div className="bg-orange-50 rounded-lg p-3">
                            <div className="text-xs text-orange-600 mb-1">ราคาคู่แข่ง</div>
                        ...
                        <div className="bg-purple-50 rounded-lg p-3">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-gray-700">ส่วนลด</span>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-600">จำนวนส่วนลด</span>
                          ...
                          {log.discount_limit_percent && (
                            <div className="text-xs text-gray-500 mt-1">
                              จำกัดส่วนลด: {log.discount_limit_percent}%
                            </div>
                          )}
                        </div>
          ...
          <Card className="mt-8 bg-gray-50">
            ...
            <CardContent>
              <div className="text-xs text-gray-600 space-y-1">
                <p>API URL: {discountLogEndpoint}</p>
                <p>จำนวนรายการในหน้า: {discountLogs.length}</p>
